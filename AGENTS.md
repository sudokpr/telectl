...
- OwnTracks heatmaps support client-side filtering by saved tags or label keywords. Preserve query prefill support for `filter`, `tag`, `q`, and `keyword`. `/owntracks/sample` serves a synthetic heatmap for visual testing without real OwnTracks logs.
