...
- OwnTracks map delivery is controlled by `OWNTRACKS_MAP_DELIVERY=file|hosted`. Preserve both modes: file mode sends the HTML attachment, hosted mode sends a local HTTP URL served from `/owntracks/map/YYYY-MM-DD`.
...
- OwnTracks date arguments support `today`, `yesterday`, `DD`, `MM-DD`, and `YYYY-MM-DD`. `DD` uses the current month and year; `MM-DD` uses the current year. Keep bot validation and help text aligned with `target_date_from_text`.
